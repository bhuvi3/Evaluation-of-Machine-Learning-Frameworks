...
conf = (SparkConf().setMaster("local[8]").setAppName("poker_hand_classification_naive_bayes").set("spark.executor.memory", "1g"))
...
#building a logistic regression rained model
train_start = time.time()
model = NaiveBayes.train(vectorized_data.cache())
train_end = time.time()
...
res_fh = open('../result/naive_bayes_classifier_result.txt','w')
res_fh.write(result)
res_fh.close()
